...
void main() {
    ...
    vec3 viewDirection = normalize(viewPos - outPosition);

    // 1. Directional Lightning
    vec3 result = CalcDirLight(dirLight, normal, viewDirection);

    // 2. All Point Lights
    for(int i = 0; i < NR_POINT_LIGHTS; i++)
        result += CalcPointLight(pointLights[i], normal, outPosition, viewDirection);

    // 3. Spotlight Light
    result += CalcSpotLight(spotLight, normal, outPosition, viewDirection);
